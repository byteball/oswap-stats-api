...
function getStateVarsForPrefix(aa_address, var_prefix_from, var_prefix_to){
...
function readAAStateVar(aa_address, var_name){
  return new Promise((resolve)=>{
    network.requestFromLightVendor('light/get_aa_state_vars', {
      address: aa_address,
    }, function(ws, request, objResponse){
...
async function updateOswapPoolTokenRates(state, onDone) {
  ...
  const pools = {};
  console.log(pool_factory_aa);
  const vars = await getStateVarsForPrefix(pool_factory_aa, 'pools.', 'pools.');
  ...
  async function scanPools() {
    for (let pool_address in pools) {
      try {
        ...
        const balances = await getBalancesByAddress(pool_address);
        ...
        const supply = await readAAStateVar(pool_address, 'supply');
        if (!supply) {
  ...
  async function getAssetAmount(balance, asset) {
    ...
    if (asset === 'base')
      ...
      const desc_hash = await readAAStateVar(asset_registry, "current_desc_" + asset);
      if (desc_hash) {
        const dec = await readAAStateVar(asset_registry, 'decimals_' + desc_hash);
